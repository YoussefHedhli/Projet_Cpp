...
employe::employe(int m, const QString& n, const QString& p, const QDate& d, const QString& e, const QString& r, const QString& mp, int et)
...
int employe::getetat() const { return etat; }
...
void employe::setetat(int et) { this->etat = et; }
...
bool employe::ajouter() {
    ...
    qDebug() << "Nom:" << nom_em;
    qDebug() << "Prenom:" << prenom_em;
    qDebug() << "DateN:" << dateN.toString("yyyy-MM-dd");
    qDebug() << "Email:" << email;
    qDebug() << "Role:" << Role;
    qDebug() << "Mot_de_passe:" << Mot_de_passe;

    // ✅ Use TO_DATE with the correct Oracle format string
    query.prepare("INSERT INTO GS_EMPLOYER (MATRICULE, NOM, PRENOM, DATEN, EMAIL, ROLE, MOT_DE_PASSE, ETAT) "
                  "VALUES (:matricule, :nom, :prenom, TO_DATE(:daten, 'YYYY-MM-DD'), :email, :role, :mot_de_passe, :etat)");

    query.bindValue(":matricule", matricule);
    query.bindValue(":nom", nom_em);
    query.bindValue(":prenom", prenom_em);
    query.bindValue(":daten", dateN.toString("yyyy-MM-dd")); // Oracle expects string in format YYYY-MM-DD
    query.bindValue(":email", email);
    query.bindValue(":role", Role);
    query.bindValue(":mot_de_passe", Mot_de_passe);
    query.bindValue(":etat", 0);
...
QString currentUserEmail = "";// Make sure this is at the top

bool employe::verifierAuthentification(const QString &email, const QString &password) {
    QSqlQuery query;
    query.prepare("SELECT ROLE, MOT_DE_PASSE FROM GS_EMPLOYER WHERE EMAIL = :email");
    query.bindValue(":email", email);
