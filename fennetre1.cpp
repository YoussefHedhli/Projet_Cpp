    ...
    , ui(new Ui::fennetre1)  // Corrected to 'fennetre1' in lowercase
{
    ...
    if (pix2.isNull()) {
        ...
        ui->label_pic2_3->setPixmap(pix2.scaled(140, 80, Qt::KeepAspectRatio, Qt::SmoothTransformation));
    }
...
void fennetre1::on_pushButton_AJemp_3_clicked() {
    ...
    QStringList validRoles = {"Administrateur", "Responsable Billetterie", "Arbitre", "Sécurité", "Médecin Sportif"};

    if (!validRoles.contains(role)) {
        QMessageBox::warning(this, "Rôle invalide", "Veuillez choisir un rôle valide : Administrateur, Responsable Billetterie, Arbitre, Sécurité, Médecin Sportif.");
        return;
    }
    ...
    if (Mot_de_passe.length() < 8) {
        QMessageBox::warning(this, "Mot de passe invalide", "Le mot de passe doit contenir au moins 8 caractères.");
        return;
    }
...
{
    QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM GS_EMPLOYER WHERE email = :email");
    ...
    query.next();
    return query.value(0).toInt() > 0;  // Si le résultat est supérieur à 0, l'email existe
}
/*
